...
describe CustomerWithSubscription do
  describe '#has_out_of_sync_user?' do
    ...
    it 'returns false if GliderPath Academy customer has same plan' do
      user = create(:user, :with_subscription)
      customer = stripe_customer_for(user, plan_id: user.subscription.plan.stripe_plan_id)
      checker = CustomerWithSubscription.new(customer)

      expect(checker).not_to have_out_of_sync_user
    end

    it 'returns true if GliderPath Academy customer has another plan' do
      user = create(:user, :with_subscription)
  ...
  describe '#update_subscription_stripe_id' do
    it 'updates local subscription stripe_id' do
      ...
      expect(user.subscription.reload.stripe_subscription_id).
  ...
  describe '#to_s' do
    it 'inspects the customer, subscription, user, and plan' do
      ...
      expect(result).to eq(<<-EOS.squish)
        ...
        #{user.subscription.plan.stripe_plan_id} in GliderPath Academy
      EOS
    end
  end
